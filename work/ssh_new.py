...
devices_filials_routers = ['10.77.176.1',
			  ...
			  '10.77.246.1',
			  '10.77.251.1']
devices_filials_9 = ["10.77.176.1", "10.77.176.2",
...
devices_filials = ["10.77.176.1", "10.77.176.2",
                   ...
                   '10.77.246.1', '10.77.246.2',
                   '10.77.251.1', '10.77.251.2', '10.77.251.3', '10.77.251.4', '10.77.251.5', '10.77.251.6']
devices=devices_filials_11
print('Устройства, на которых будет выполняться скрипт=', devices)
# devices=['10.77.30.3']
commands = ['conf ter', 'clock timezone +11 11 0', 'ntp update-calendar','end', 'wr']
...
# commands = ['conf ter', 'no ntp authentication-key 1 md5 15060E1F10243F34 7', 'no ntp authenticate', 'no ntp trusted-key 1', 'ntp master 2', 'no ntp server 80.73.69.174 prefer', 'no ntp server 10.77.30.1 prefer', 'no ip access-list standard ntp_access', 'ip access-list standard ntp_access', 'permit 10.77.0.0 0.0.255.255', 'permit 10.100.0.0 0.0.0.255', 'permit 10.200.0.0 0.0.0.255', 'permit 172.26.160.0 0.0.1.255', 'no ip access-list standard ntp_peers', 'ip access-list standard ntp_peers', 'permit 10.77.0.0 0.0.255.255', 'permit 172.26.160.0 0.0.1.255', 'permit 10.100.0.0 0.0.0.255', 'permit 10.200.0.0 0.0.0.255', 'ntp access-group serve-only ntp_access', 'ntp access-group peer ntp_peers', 'ntp update-calendar', 'clock update-calendar','clock update-calendar','ntp server 10.77.30.1 prefer','end', 'wr']
